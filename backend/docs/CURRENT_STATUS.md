...
- [x] 前端视觉重构预览版：详情页返回按钮 / 首页三档轮播 / 管理页 5 源线性滑块 / 悬浮+侧边两种导航切换（2026-09-18）
- [x] 前端视觉第二round：5 套暖色背景方案 + 背景色下拉切换 / 导航整合进胶囊（logo+链接+主题+用户）/ 返回按钮 hover 只展字不渲染背景 / 轮播收敛为淡入式多色渐变+装饰纹理（2026-09-18）
- [x] 前端视觉第三round：删 header 与侧边导航，全站只留居中悬浮胶囊 / 轮播删文字进度水印（01/05）保留圆点 / 返回按钮图标与文字改块容器 div（2026-09-19）
- [x] 前端视觉第四round：文章改全屏阅读页（新 tab 打开 /read/:id，去返回按钮加关闭按钮）/ 轮播 1/4/5 号改奶油低饱和配色 / 主题补第 6 套「纯奶·亮」/ 新增亮度滑条（localStorage 持久化，默认 50%）（2026-09-19）
- [x] 前端视觉第五round：亮度滑条并入主题按钮面板（胶囊删独立滑条）/ 修默认亮度 0→50（Number(null) 判空）/ 深色模式轮播换低饱和暗色渐变 / 删导航最左侧 logo / 新增跨 tab 主题同步（storage 事件监听）/ 文章详情页标题卡加 head-row 关按钮、删 40px 顶部间距对齐首页轮播（2026-09-19）
- [x] 前端视觉第六round：登录页接入主题系统（整页写死橙棕渐变全部改为 CSS 变量，随 6 套背景方案与深色模式自动切换，与全站一致）/ 左栏品牌区改从容布局（加大留白与行高）并新增真实数据徽标（来源/库内文章/覆盖国家数，来自公开 /sources/overview，后端不可用时自动隐藏，不影响登录）（2026-09-19）
- [x] 前端视觉第七round：登录页左栏精简——删除 logo 与品牌名，保留加粗放大的标语「全球 AI 动态聚合，让前沿一目了然」+ 三条卖点列表（多源采集/AI 筛选/地域视角）+ 三张真实数据统计卡（接入来源/库内文章/覆盖国家，公开 /sources/overview），右栏功能区域不动（2026-09-19，用户框选确认）
