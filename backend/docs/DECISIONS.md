...
### D13 工具层约定：codegraph / code-review-graph 为可选软约束
- **决策**：`codegraph`（本机代码索引）与 `code-review-graph`（代码审理）是纯本机、gitignored 的可选加速工具，**不是团队硬性依赖**。团队文档只写软约束（见 `AI_AGENT_GUIDE.md`「AI 辅助工具（可选）」），不写硬性引用；未安装任何成员不受影响。
- **原因**：团队多成员共库，不是所有人都装了这些工具；若把工具写进硬性流程，会导致「我没装这步怎么过」的摩擦。工具价值在于快速取源码/查 diff 风险，但保证真相仍来自 docs 与代码本身。**约定按「判据 / 职责 / 边界」写成通用条款，随工具持续演进而机制不随工具名变动——新增任何本机工具只需登记一行，不重写本节。**
- **影响**：`AI_AGENT_GUIDE.md` 新增图例小节；工具产生的结论（新问题/风险）必须落到 `TECH_DEBT.md` 或 PR 描述，不可只有工具层可见。

## 待决策

- （下轮补充：文章清洗/摘要 stage、评分接入、任务 retry/cancel、多源扩展）
- **内容合规：全文入库 vs 摘要+跳转**——当前 InfoQ 源经 trafilatura 抓**全文**入库并在前端展示（D10）。自用无碍；若平台对外开放，全文转载第三方内容存在版权风险。候选方案：a) 只存/只展示摘要，详情跳转原文（聚合类产品常规做法）；b) 维持全文但仅登录自用、不公开分发；c) 优先接有授权全文的官方渠道（RSS 全文输出/合作 API）。定案前不阻塞开发，但**公开上线前必须定**。
