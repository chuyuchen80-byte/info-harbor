...
修改/新增接口或文件结构后，若项目有 .codegraph/，运行 codegraph sync。
...
> 下述约定为**通用条款**：适用于任何本机工具（skill / MCP / CLI），
> 与具体工具名解耦。团队各成员安装情况不必一致——工具是**可选的**，
> 未安装不影响任何开发流程，缺省自然退化为普通阅读 / 自查。

**1. 判据：按特征判定，不按名字枚举**

工具是否可用，以「本机是否存在其标记目录」为准（例如 `.codegraph/`、
`.code-review-graph/`）。存在即用，不存在则跳过该环节，绝不因缺工具而阻塞流程。

**2. 职责：按时刻判定，不写死工具**

- 理解 / 改动代码前 → 先用可用的**索引类**工具取符号、调用路径、影响范围；
  改动完成后 → 用可用的**自查类**工具扫 diff 风险与测试缺口，并同步本机索引。
- 没有索引工具时，退化为直接读文件 / grep，行为一致，只是慢一些。

**3. 边界：硬性约束只针对输出，与工具无关**

- docs 同步 / 测试 / 状态真实等硬性约束永远成立，不因用没用什么工具而改变。
- 工具得出的新问题 / 风险，**摘下摘不掉**：必须进 `TECH_DEBT.md` 或 PR 描述，
  不可只停留在工具层。

> **新增任何本机工具时**，只需在本节末尾登记一行「工具名 → 标记目录」，
> 本节机制本身无需改动。
